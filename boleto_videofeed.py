...
def overlay_image(background, overlay, x, y):
    ...
    # Se a imagem de sobreposição tiver canal alfa (transparência)
    if overlay.shape[2] == 4:
        # Separar os canais de cores e o alfa
        overlay_img = overlay[:, :, :3]
        overlay_mask = overlay[:, :, 3:]
        # Obter a região do background onde a imagem será sobreposta
        bg_region = background[y:y+overlay_img.shape[0], x:x+overlay_img.shape[1]]
        # Usar o canal alfa como máscara para combinar as imagens
        ...
        # Se não houver canal alfa, simplesmente copia a imagem
        background[y:y+overlay.shape[0], x:x+overlay.shape[1]] = overlay
    return background

def draw_bounding_boxes(img, boxes):
    for box in boxes:
        x_min, y_min, x_max, y_max, _, _ = box  # Ignoramos confidence e class_id
        # Desenha as bounding boxes na imagem sem texto
...
def monitor_handler(device, msg):
    if "image" in msg:
        ...
        # Convertendo os bytes para um array numpy
        nparr = np.frombuffer(jpeg_bytes, np.uint8)
        img = cv2.imdecode(nparr, cv2.IMREAD_COLOR)

        # Redimensionando a imagem para a tela de exibição
        img = cv2.resize(img, (DISPLAY_WIDTH, DISPLAY_HEIGHT))

        # Verifica se há bounding boxes no JSON
        if "boxes" in msg:
            boxes = msg["boxes"]
            # Redimensiona as bounding boxes de acordo com a nova resolução
            ...
            for box in resized_boxes:
                ...
                # Redimensionar a imagem para caber na bounding box
                overlay_img_resized = cv2.resize(overlay_img, (x_max - x_min, y_max - y_min))

                # Sobrepor a imagem na posição da bounding box
                img = overlay_image(img, overlay_img_resized, x_min, y_min)

        # Exibir a imagem sem textos e sem espera prolongada
        cv2.imshow('Video Feed', img)
        cv2.waitKey(1)

    # Exibe o conteúdo do JSON sem a imagem
    print(msg)
